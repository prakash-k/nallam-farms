...
document.addEventListener('DOMContentLoaded', () => {
    ...
    tabLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            // If we are inside an iframe, let the parent know we want to navigate to textiles
            if (window.parent && window.parent !== window) {
                if (targetTab === 'textiles') {
                    e.preventDefault();
                    window.parent.location.hash = '#/catalog';
                    window.parent.postMessage({ type: 'navigate', route: '/catalog' }, '*');
                    return;
                }
            }
    ...
    textileDetailBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            // If we are inside an iframe, let the parent know to route to the detailed product
            if (window.parent && window.parent !== window) {
                window.parent.location.hash = '#/product?id=' + productId;
                window.parent.postMessage({ type: 'navigate', route: '/product?id=' + productId }, '*');
                return;
            }
    ...
    const l10nDict = {
        ...
        ja: {
            ...
            "about_body": "私たちの旅は、「誰もがクリーンで化学物質を含まない食品を手にする権利がある」というシンプルな信念から始まりました。古くからの自然農法の知恵を尊重し、厳格な品質基準を統合することで、すべての収穫物が自然のビタミン、本物の味、前して絶対的な新鮮さを保てるようにしています。",
            ...
            "card_textile_p10_desc": "丈夫な100%厚手コットンキャンバス of ペットベッド。取り外し可能なジッパー付きカバーとサポート力のあるエコポリエステル中綿。",
